...
def build_prompt(query: str, chunks: list[dict]) -> str:
    """
    Build the full prompt from query + retrieved chunks.
    """
    context_parts = []
...
def summarize(query: str, chunks: list[dict]) -> dict:
    """
    Generate a structured clinical summary using Gemini.
    Returns a dict with summary text and source citations.
    """
    if not chunks:
        return {
    ...
    print(f"  [Summarizer] Calling {CHAT_MODEL}...")
