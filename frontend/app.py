"""
frontend/app.py
Clinical Review Summarizer — Timeline-aware UI
...
.section-divider-sub   { font-size: 11px; color: var(--ink4); font-family: 'IBM Plex Mono', monospace; margin-top: 3px; }
...
.pill.purple { background: var(--purple-light);border-color: rgba(106,27,154,0.20); color: var(--purple); }
...
/* ── Dates panel ── */
.dates-panel { background: var(--surface); border: 1px solid var(--border); border-left: 3px solid var(--purple); border-radius: 0 var(--r) var(--r) 0; padding: 14px 18px; margin-bottom: 16px; box-shadow: var(--shadow-sm); }
.date-row { display: flex; gap: 10px; align-items: flex-start; padding: 7px 0; border-bottom: 1px solid var(--border); }
.date-row:last-child { border-bottom: none; }
.date-badge { font-size: 11px; font-weight: 600; color: var(--purple); font-family: 'IBM Plex Mono', monospace; background: var(--purple-light); padding: 2px 8px; border-radius: 4px; white-space: nowrap; flex-shrink: 0; margin-top: 2px; }
.date-event { font-size: 13px; color: var(--ink2); line-height: 1.5; }
.date-raw   { font-size: 11px; color: var(--ink4); font-family: 'IBM Plex Mono', monospace; margin-top: 2px; }

/* ── Progression narrative ── */
.progression-card { background: var(--blue-light); border: 1px solid rgba(21,101,192,0.18); border-left: 3px solid var(--blue); border-radius: 0 var(--r) var(--r) 0; padding: 16px 20px; margin-bottom: 16px; }
.progression-text { font-size: 14px; color: var(--ink2); line-height: 1.85; }

/* ── Condition card ── */
.cond-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); margin-bottom: 18px; box-shadow: var(--shadow-sm); overflow: hidden; }
.cond-header { padding: 14px 18px 10px; border-bottom: 1px solid var(--border); }
.cond-name { font-size: 15px; font-weight: 700; color: var(--ink); }
.cond-meta-row { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 7px; }
.cond-badge { font-size: 10px; font-weight: 600; padding: 3px 10px; border-radius: var(--r-pill); font-family: 'IBM Plex Mono', monospace; }
.cond-badge.active     { background: var(--red-light);    color: var(--red);    border: 1px solid rgba(198,40,40,0.25); }
.cond-badge.developing { background: var(--amber-light);  color: var(--amber);  border: 1px solid rgba(245,124,0,0.25); }
.cond-badge.resolving  { background: var(--green-light);  color: var(--green);  border: 1px solid rgba(46,125,50,0.25);  }
.cond-badge.chronic    { background: var(--purple-light); color: var(--purple); border: 1px solid rgba(106,27,154,0.25); }
.cond-badge.at_risk    { background: var(--blue-light);   color: var(--blue);   border: 1px solid rgba(21,101,192,0.25); }
.cond-prob { font-size: 11px; color: var(--ink3); font-family: 'IBM Plex Mono', monospace; padding: 3px 10px; background: var(--surface2); border-radius: var(--r-pill); border: 1px solid var(--border-em); }

/* ── Timeline strip ── */
.tl-strip { padding: 16px 18px 14px; background: var(--surface2); border-top: 1px solid var(--border); }
.tl-strip-title { font-size: 9px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; font-family: 'IBM Plex Mono', monospace; color: var(--teal); margin-bottom: 14px; }
.tl-step { display: flex; flex-direction: column; align-items: center; text-align: center; }
.tl-dot { width: 22px; height: 22px; border-radius: 50%; border: 2px solid var(--surface); display: flex; align-items: center; justify-content: center; font-size: 10px; margin-bottom: 7px; }
.tl-dot.trigger    { background: var(--blue);   }
.tl-dot.onset      { background: var(--amber);  }
.tl-dot.peak       { background: var(--red);    }
.tl-dot.resolution { background: var(--green);  }
.tl-lbl  { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; font-family: 'IBM Plex Mono', monospace; color: var(--ink4); margin-bottom: 4px; }
.tl-date { font-size: 12px; font-weight: 600; color: var(--ink2); font-family: 'IBM Plex Mono', monospace; line-height: 1.35; }
.tl-date.nd { color: var(--ink4); font-style: italic; font-size: 11px; font-weight: 400; }
.tl-conf { font-size: 10px; color: var(--ink4); margin-top: 3px; font-family: 'IBM Plex Mono', monospace; }

/* ── Cond body ── */
.cond-body { padding: 12px 18px 16px; }
.cond-trigger  { font-size: 12px; color: var(--blue); font-family: 'IBM Plex Mono', monospace; margin-bottom: 7px; }
.cond-evidence { font-size: 13px; color: var(--ink3); line-height: 1.6; margin-bottom: 5px; }
.cond-citation { font-size: 11px; color: var(--ink4); font-family: 'IBM Plex Mono', monospace; }
.cond-basis { font-size: 12px; color: var(--ink3); background: var(--blue-light); border-left: 2px solid var(--blue); padding: 7px 12px; border-radius: 0 var(--r-sm) var(--r-sm) 0; margin-top: 10px; line-height: 1.6; }

/* ── Precautions / lifestyle / tests ── */
...
.test-when { font-size: 11px; color: var(--purple); font-family: 'IBM Plex Mono', monospace; margin-top: 3px; font-weight: 600; }
.test-freq { font-size: 10px; color: var(--teal); font-family: 'IBM Plex Mono', monospace; margin-top: 2px; }
...
.welcome-sub   { font-size: 14px; color: var(--ink3); line-height: 1.9; max-width: 460px; margin: 0 auto; }
...
.hist { padding: 8px 10px; border-radius: 8px; margin-bottom: 2px; }
...
def call_summarize(query, n_results, use_mmr, source_filter=None):
    ...
    if not r.ok: raise Exception(r.json().get("detail", "Summarize error"))
    return r.json()
...
def call_predict(query, n_results, use_mmr, source_filter=None):
    ...
    if not r.ok: raise Exception(r.json().get("detail", "Predict error"))
    return r.json()
...
def risk_card_cls(lvl):
    return {"low": "card-risk-low", "moderate": "card-risk-moderate",
            "high": "card-risk-high"}.get(lvl.lower(), "card-risk-unknown")
...
def is_nd(val: str) -> bool:
    """True if the value means 'not documented / not applicable'."""
    if not val: return True
    return val.strip().lower() in (
        "not documented", "not applicable", "n/a", "",
        "ongoing / chronic", "ongoing/chronic", "ongoing", "chronic"
    )
...
def render_card(tag_label, content, warn=False, green=False, pink=False):
    ...
    st.markdown(f'<div class="card"><div class="{cls}">{tag_label}</div>'
                f'<div class="{body_cls}">{text}</div></div>', unsafe_allow_html=True)


# ── Timeline step (one column) ─────────────────────────────────────────────────
def render_tl_step(dot_cls: str, label: str, date_val: str, confidence: str = ""):
    date_cls  = "tl-date nd" if is_nd(date_val) else "tl-date"
    disp      = date_val if date_val else "Not documented"
    conf_html = f'<div class="tl-conf">{confidence} confidence</div>' if confidence else ""
    st.markdown(
        f'<div class="tl-step">'
        f'<div class="tl-dot {dot_cls}"></div>'
        f'<div class="tl-lbl">{label}</div>'
        f'<div class="{date_cls}">{disp}</div>'
        f'{conf_html}</div>',
        unsafe_allow_html=True,
    )
...
def render_condition(cond: dict):
    name     = cond.get("condition", "Unknown")
    status   = cond.get("status", "at_risk").lower().replace(" ", "_")
    prob     = cond.get("probability", "?")
    trigger  = cond.get("trigger", "")
    evidence = cond.get("supporting_evidence", "")
    citation = cond.get("citation", "")
    tl       = cond.get("timeline", {})

    trigger_date    = tl.get("trigger_date", "Not documented")
    onset_window    = tl.get("onset_window", "Not documented")
    peak_date       = tl.get("peak_date", "Not documented")
    resolution_date = tl.get("resolution_date", "Not documented")
    ...
    st.markdown(
        ...
        f'<div class="cond-name">{name}</div>'
        f'<div class="cond-meta-row">'
        f'<span class="cond-badge {status}">{status.replace("_"," ").upper()}</span>'
        f'<span class="cond-prob">Probability: {prob}</span>'
        f'</div></div>',
        unsafe_allow_html=True,
    )

    # Timeline strip — 4 columns, one st.markdown per step
    st.markdown(
        '<div class="tl-strip"><div class="tl-strip-title">📅 Disease Timeline</div>',
        unsafe_allow_html=True,
    )
    c1, c2, c3, c4 = st.columns(4)
    with c1: render_tl_step("trigger",    "Trigger / Dx",    trigger_date)
    with c2: render_tl_step("onset",      "Expected Onset",  onset_window)
    with c3: render_tl_step("peak",       "Peak / Worst",    peak_date)
    with c4: render_tl_step("resolution", "Resolution",      resolution_date, confidence)
    st.markdown('</div>', unsafe_allow_html=True)

    # Body
    st.markdown('<div class="cond-body">', unsafe_allow_html=True)
    if trigger:
        st.markdown(f'<div class="cond-trigger">🔍 <strong>Trigger:</strong> {trigger}</div>', unsafe_allow_html=True)
    if evidence:
        st.markdown(f'<div class="cond-evidence">{evidence}</div>', unsafe_allow_html=True)
    if citation:
        st.markdown(f'<div class="cond-citation">{citation}</div>', unsafe_allow_html=True)
    if basis and not is_nd(basis):
        st.markdown(f'<div class="cond-basis">📖 {basis}</div>', unsafe_allow_html=True)
    st.markdown('</div></div>', unsafe_allow_html=True)  # close cond-body + cond-card


# ── Dates extraction panel ─────────────────────────────────────────────────────
def render_dates_panel(doc_dates: list, reference_date: str):
    if not doc_dates: return
    ref_html = ""
    if reference_date and not is_nd(reference_date):
        ref_html = (f'<span class="pill purple" style="display:inline-block;margin-bottom:10px;">'
                    f'📌 Reference: {reference_date}</span>')
    st.markdown(
        f'<div class="dates-panel">'
        f'<div class="tag purple">Dates extracted from document</div>'
        f'{ref_html}',
        unsafe_allow_html=True,
    )
    for d in doc_dates:
        st.markdown(
            ...
            f'<span class="date-badge">{d.get("interpreted_date","")}</span>'
            f'<div><div class="date-event">{d.get("event","")}</div>'
            f'<div class="date-raw">"{d.get("raw_text","")}"</div></div>'
            f'</div>',
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_prediction_section(p: dict):
    ...
    # Dates panel
    render_dates_panel(p.get("document_dates", []), p.get("reference_date", ""))

    # Progression narrative
    prog = p.get("disease_progression_summary", "")
    if prog and not is_nd(prog):
        st.markdown(
            f'<div class="progression-card">'
            f'<div class="tag">Disease Progression Narrative</div>'
            f'<div class="progression-text">{prog}</div></div>',
            unsafe_allow_html=True,
        )

    # Conditions with timelines
    conditions = p.get("predicted_conditions", [])
    st.markdown('<div class="tag purple" style="margin-top:6px;margin-bottom:10px;">Predicted Conditions &amp; Timelines</div>', unsafe_allow_html=True)
    if conditions:
        for cond in conditions:
            render_condition(cond)
    else:
        st.markdown('<div class="card"><div class="muted">No specific conditions predicted from current data.</div></div>', unsafe_allow_html=True)

    # Precautions + Lifestyle
    col1, col2 = st.columns(2)
    with col1:
        precs = p.get("precautions", [])
        st.markdown('<div class="card"><div class="tag green">Precautions</div>', unsafe_allow_html=True)
        if precs:
            for i, pr in enumerate(precs):
                st.markdown(f'<div class="prec-item"><div class="prec-num">{i+1}</div><div class="prec-text">{pr}</div></div>', unsafe_allow_html=True)
        else:
    ...
    # Follow-up tests — with recommended_date
    ...
    if tests:
        ...
        for t in tests:
            rec  = t.get("recommended_date", "")
            when = f'<div class="test-when">📅 By: {rec}</div>' if rec and not is_nd(rec) else ""
            st.markdown(
                f'<div class="test-row">'
                f'<div class="test-name">🧪 {t.get("test","")}</div>'
                f'{when}'
                f'<div class="test-freq">Ongoing: {t.get("frequency","As advised")}</div>'
    ...
    # Reasoning + Data gaps
    c3, c4 = st.columns(2)
    with c3: render_card("Clinical Reasoning", p.get("reasoning", ""))
    with c4: render_card("Data Gaps",          p.get("data_gaps", ""), warn=True)

    # Sources
    srcs = p.get("sources_used", [])
    if srcs:
        chips = "".join(f'<span class="chip">📄 {s}</span>' for s in srcs if s)
        st.markdown(f'<div class="card"><div class="tag purple">Sources</div>{chips}</div>', unsafe_allow_html=True)

    # Disclaimer
    st.markdown(
        f'<div class="disclaimer">⚠ <strong>Medical Disclaimer:</strong> {p.get("disclaimer","")}</div>',
        unsafe_allow_html=True,
    )


# ── Session state ──────────────────────────────────────────────────────────────
for k, v in {"query": "", "summary": None, "prediction": None,
             "error": None, "last_uploaded": None}.items():
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown('<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,0.28);margin-bottom:10px;'
                'letter-spacing:2px;text-transform:uppercase;font-family:\'IBM Plex Mono\',monospace;">⚙ Settings</div>',
                unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,0.28);margin-bottom:10px;'
                'letter-spacing:2px;text-transform:uppercase;font-family:\'IBM Plex Mono\',monospace;">📄 Upload</div>',
                unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<div style="font-size:9px;font-weight:700;color:rgba(255,255,255,0.28);margin-bottom:10px;'
                'letter-spacing:2px;text-transform:uppercase;font-family:\'IBM Plex Mono\',monospace;">🕐 Recent</div>',
                unsafe_allow_html=True)
    if not history:
        ...
        for h in history[:6]:
            q    = h.get("query", "")
            rref = h.get("reference_date", "")
            meta = f'{fmt_time(h.get("timestamp",""))} · {rref}' if rref and not is_nd(rref) else fmt_time(h.get("timestamp",""))
            st.markdown(
                f'<div class="hist"><div class="hist-q">📋 {q[:44]}{"..." if len(q)>44 else ""}</div>'
                f'<div class="hist-m">{meta}</div></div>',
                unsafe_allow_html=True,
            )
...
<div class="subtitle-banner">
    ...
    Dates in your document are extracted automatically — every predicted condition gets a
    real <strong>onset window, peak date, and resolution estimate</strong> anchored to actual documented events.
</div>
""", unsafe_allow_html=True)
st.markdown(
    '<div style="font-size:9px;color:var(--ink4);margin-bottom:24px;font-family:\'IBM Plex Mono\',monospace;'
    'letter-spacing:2px;text-transform:uppercase;">RAG · GROQ LLAMA 3.3 · CHROMADB · TIMELINE-AWARE</div>',
    unsafe_allow_html=True,
)

EXAMPLES = ["What is the disease timeline?", "What is the primary diagnosis?",
            "What medications were prescribed?", "What comorbidities are present?"]
st.markdown('<div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--ink4);'
            'font-family:\'IBM Plex Mono\',monospace;margin-bottom:8px;">Try an example</div>', unsafe_allow_html=True)
...
<div class="scope-card">
    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:2px;
        color:var(--teal);font-family:'IBM Plex Mono',monospace;margin-bottom:6px;">📂 Search Scope</div>
...
query = st.text_area("Your question", value=st.session_state.query,
    placeholder="Ask about diagnosis, disease timeline, risk factors, lab values, progression...",
    height=110)

bcol, ccol = st.columns([5, 1])
with bcol:
    submit = st.button("🔍  Analyze — Summarize & Predict Timeline",
                       disabled=not query.strip() or not online, use_container_width=True)
with ccol:
...
if submit and query.strip():
    ...
    with ph:
        ...
        <div class="loader">
            ...
            <div class="loader-s">extracting dates · summarizing · building timeline</div>
            <div class="loader-steps">
                ...
                <div class="lstep"><div class="lstep-dot"></div><div class="lstep-lbl">Timeline</div></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    try:
...
# ── Results ────────────────────────────────────────────────────────────────────
    ...
    lvl = p.get("risk_level", "unknown")
    ref = p.get("reference_date", "")
    fp  = f'<span class="pill">{source_filter}</span>' if source_filter else '<span class="pill">All docs</span>'
    ref_pill = f'<span class="pill teal">📅 {ref}</span>' if ref and not is_nd(ref) else ""
    ...
    <div class="section-divider">
        ...
        <div><div class="section-divider-title">Clinical Summary</div>
        <div class="section-divider-sub">structured · cited · grounded in your document</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="section-divider">
        <div class="section-divider-icon">📅</div>
        <div><div class="section-divider-title">Timeline-aware Risk Prediction</div>
        <div class="section-divider-sub">date-anchored · onset windows · peak · resolution · precautions</div></div>
    </div>
    """, unsafe_allow_html=True)
...
# ── Welcome ────────────────────────────────────────────────────────────────────
    ...
    <div class="welcome">
        ...
        <div class="welcome-sub">
            Upload a clinical document with dates, lab results, or diagnoses.<br>
            Get a full <strong>Summary</strong> + <strong>Timeline-aware Risk Prediction</strong> —<br>
            onset windows, peak dates, and resolution estimates anchored to real document dates.
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    for col, (bg, color, ico, lbl, desc) in zip([col1, col2, col3], [
        ("#E3EDF9", "#1565C0", "🔍", "Retrieve",
         "Semantic search across indexed clinical chunks using sentence-transformers and MMR re-ranking"),
        ("#E0F2F0", "#00796B", "📋", "Summarize",
         "Groq Llama 3.3 produces a structured summary: overview, findings, diagnosis, recommendations"),
        ("#F3E5F5", "#6A1B9A", "📅", "Timeline",
         "Dates are extracted from the document and used to anchor every condition's onset, peak, and resolution window"),
    ]):
        with col:
            st.markdown(f"""
            <div class="card" style="text-align:center;padding:28px 16px;">
                <div style="width:44px;height:44px;border-radius:11px;background:{bg};
                    display:flex;align-items:center;justify-content:center;font-size:20px;margin:0 auto 14px;">{ico}</div>
                <div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;
                    font-family:'IBM Plex Mono',monospace;color:{color};margin-bottom:8px;">{lbl}</div>
